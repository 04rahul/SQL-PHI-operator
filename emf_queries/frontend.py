...
from generator import emf_genrator
import subprocess
import time
import re
   ...
   if submitted:
    if not select_arg or not n_arg or not v_arg or not f_arg :
       ...
       if set(select_arg_list) != set(v_grp_attr_list):
            ...
            f_arg_list = [x.strip() for x in len(f_arg.split(',')[-1].split('_')[0])+1]
            predi_arg_list = [x.strip() for x in predicates_arg.split(',')]
            having_list = [x.strip() for x in having_arg.split(',')]


    emf_genrator(s=select_arg_list,n=n_arg,v=v_grp_attr_list,f = f_arg_list,predicates = predi_arg_list,having = having_list)
...
if uploaded_file is not None:
    ...
    # st.write(bytes_data)
        ...
        emf_genrator(s=select_arg_list,n=n_arg,v_list=v_grp_attr_list,f_list = f,pred_list = values,having_list = having_list)
        time.sleep(2)
        from _generated import query
